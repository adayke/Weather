...
})({"b33L7":[function(require,module,exports) {
"use strict";
var HMR_HOST = null;
var HMR_PORT = null;
...
parcelHelpers.export(exports, "showNow", ()=>showNow
);
...
parcelHelpers.export(exports, "createFavorite", ()=>createFavorite
);
...
async function showNow(serverUrl, cityName) {
    let weatherData = await jsonWeather(serverUrl, cityName);
    try {
        _viewJs.UI_ELEMENTS.TEMP_NOW.textContent = `${kelvinToCelsius(weatherData.main.temp)}°`;
        _viewJs.UI_ELEMENTS.CITY_NOW.textContent = weatherData.name;
    } catch (e) {
        alert(weatherData.message);
    }
}
async function showDetails(serverUrl, cityName) {
    let weatherDetails = await jsonWeather(serverUrl, cityName);
    _viewJs.UI_ELEMENTS.DETAILS_TEMP.textContent = `${kelvinToCelsius(weatherDetails.main.temp)}°`;
    _viewJs.UI_ELEMENTS.DETAILS_FEELS.textContent = `${kelvinToCelsius(weatherDetails.main.feels_like)}°`;
    _viewJs.UI_ELEMENTS.DETAILS_WEATHER.textContent = weatherDetails.weather[0].main;
    _viewJs.UI_ELEMENTS.DETAILS_CITY.textContent = weatherDetails.name;
    const sunriseTime = getTime(weatherDetails.sys.sunrise);
    const sunsetTime = getTime(weatherDetails.sys.sunset);
    _viewJs.UI_ELEMENTS.DETAILS_SUNRISE.textContent = sunriseTime;
    _viewJs.UI_ELEMENTS.DETAILS_SUNSET.textContent = sunsetTime;
}
function showForecast(serverUrl, cityName) {
    jsonWeather(serverUrl, cityName).then((weather)=>{
        _viewJs.UI_ELEMENTS.FORECAST_CITY.textContent = weather.city.name;
        for(let i = 0; i < _viewJs.UI_ELEMENTS.FORECAST_ITEMS.length; i++){
            let cityWeather = weather.list[i].weather[0].main;
            _viewJs.UI_ELEMENTS.FORECAST_DATE.forEach((item)=>{
                item.textContent = getDate(weather.list[0].dt_txt);
            });
            _viewJs.UI_ELEMENTS.FORECAST_TEMP.forEach((item)=>{
                item.firstElementChild.textContent = `${kelvinToCelsius(weather.list[i].main.temp)}°`;
            });
            _viewJs.UI_ELEMENTS.FORECAST_FEELS.forEach((item)=>{
                item.firstElementChild.textContent = `${kelvinToCelsius(weather.list[i].main.feels_like)}°`;
            });
            _viewJs.UI_ELEMENTS.FORECAST_WEATHER.forEach((item)=>{
...
function addFavorite() {
    let cityName = _viewJs.UI_ELEMENTS.CITY_NOW.textContent;
    try {
        if (_storageJs.favoriteList.includes(cityName)) throw new Error('this city is already on the list');
        else {
            _storageJs.favoriteList.push(cityName);
            localStorage.setItem('favorites', JSON.stringify(_storageJs.favoriteList));
...
function createFavorite(cityName) {
    ...
    _viewJs.UI_ELEMENTS.FAVORITE_LIST.append(newFavoriteItem);
    newFavoriteItem.firstElementChild.addEventListener('click', _viewJs.showWeatherFromFavorite);
...
function deleteFavorite(event) {
    ...
    const favItemId = _storageJs.favoriteList.indexOf(cityName);
    _storageJs.favoriteList.splice(favItemId, 1);
    localStorage.setItem('favorites', JSON.stringify(_storageJs.favoriteList));
    favItem.remove();
}
...
},{"./view.js":"2GA9o","./storage.js":"j1l1C","@parcel/transformer-js/src/esmodule-helpers.js":"4TSZV"}],"2GA9o":[function(require,module,exports) {
var parcelHelpers = require("@parcel/transformer-js/src/esmodule-helpers.js");
parcelHelpers.defineInteropFlag(exports);
parcelHelpers.export(exports, "UI_ELEMENTS", ()=>UI_ELEMENTS
);
parcelHelpers.export(exports, "showWeatherFromFavorite", ()=>showWeatherFromFavorite
);
...
function showWeatherFromFavorite(event) {
    _mainJs.showNow(_mainJs.SERVER.URL_MAIN, event.currentTarget.textContent);
    _mainJs.showDetails(_mainJs.SERVER.URL_MAIN, event.currentTarget.textContent);
    _mainJs.showForecast(_mainJs.SERVER.URL_FORECAST, event.currentTarget.textContent);
    event.preventDefault();
}
function showWeatherFromSearch(event) {
    _mainJs.showNow(_mainJs.SERVER.URL_MAIN, UI_ELEMENTS.INPUT.value);
    _mainJs.showDetails(_mainJs.SERVER.URL_MAIN, UI_ELEMENTS.INPUT.value);
    _mainJs.showForecast(_mainJs.SERVER.URL_FORECAST, UI_ELEMENTS.INPUT.value);
    event.preventDefault();
}
UI_ELEMENTS.SUBMIT.addEventListener('click', showWeatherFromSearch);
UI_ELEMENTS.ADD_TO_FAVORITE.addEventListener('click', _mainJs.addFavorite);
for (let btn of UI_ELEMENTS.FAVORITE_CITIES)btn.addEventListener('click', showWeatherFromFavorite);
for (let btn1 of UI_ELEMENTS.DELETE_FAVORITE)btn1.addEventListener('click', _mainJs.deleteFavorite);
_storageJs.showStorage();

},{"./main.js":"bDbGG","./storage.js":"j1l1C","@parcel/transformer-js/src/esmodule-helpers.js":"4TSZV"}],"j1l1C":[function(require,module,exports) {
var parcelHelpers = require("@parcel/transformer-js/src/esmodule-helpers.js");
parcelHelpers.defineInteropFlag(exports);
parcelHelpers.export(exports, "favoriteList", ()=>favoriteList
);
parcelHelpers.export(exports, "showStorage", ()=>showStorage
);
var _mainJs = require("./main.js");
const favoriteList = [];
function showStorage() {
    ...
    favoriteCities.forEach((item)=>{
        _mainJs.createFavorite(item);
    });
}

},{"./main.js":"bDbGG","@parcel/transformer-js/src/esmodule-helpers.js":"4TSZV"}],"4TSZV":[function(require,module,exports) {
...
},{}]},["b33L7","bDbGG"], "bDbGG", "parcelRequire94c2")
